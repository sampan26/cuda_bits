...
matmul_kernel_v4(int M, int N, int K, bf16* C, const CUtensorMap* tensorMapA, const CUtensorMap* tensorMapB) {
...
void matmul_v4(int M, int N, int K, bf16 *A, bf16 *B, bf16 *C) {
  ...
  kernel<<<(M/BM) * (N/BN), NUM_THREADS, smem_size>>>(M, N, K, C, d_tma_map_A, d_tma_map_B);
}
