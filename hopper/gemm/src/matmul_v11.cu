    ...
    const __grid_constant__ CUtensorMap tensorMapC) 
{
    ...
    __shared__ __align__(8) uint64_t full_barrier[PIPE], empty_barrier[PIPE], pingpong[2][NUM_CONSUMERS];
    ...
    if (threadIdx.x == 0) {
        ...
        for (int i = 0; i < NUM_CONSUMERS; ++i) {
            init_barriers(&pingpong[0][i], 1);
            init_barriers(&pingpong[1][i], 1);
        }
    }
    ...
    if (wg_idx == 0) {
        ...
        if (wg_idx == 1 && tid == 0) {
            arrive(&pingpong[0][1-wg_idx]); // ping mma wg 0
            arrive(&pingpong[1][1-wg_idx]); // ping mma wg 0
        }
        ...
        for (int tile_idx=cluster_id; tile_idx<num_blocks; tile_idx+=NUM_SM/CLUSTERS) {
            ...
            wait(&pingpong[0][wg_idx], pingpong_phase);
            int prev_lane = pipe_lane;
            {
                if (pipe_lane == PIPE) { pipe_lane = 0; p ^= 1; }
            ...
            for (int k_tile = 1; k_tile < num_tiles_k; ++k_tile) {
                ...
                warpgroup_commit_batch();
                warpgroup_wait<1>();
                if (tid < CLUSTERS) arrive_cluster(&empty_barrier[prev_lane], tid);
                prev_lane = pipe_lane;
                ++pipe_lane;
            }
            ...
            if (tid == 0) {
                arrive(&pingpong[0][1-wg_idx], 1);
            }

            wait(&pingpong[1][wg_idx], pingpong_phase);
...
void matmul_v11(int M, int N, int K, bf16 *A, bf16 *B, bf16 *C) {
    ...
    auto* kernel = matmul_kernel_v11<BM,BN,BK,NUM_THREADS,NUM_CONSUMERS, PIPE,NUM_SM,CLUSTER_M,CLUSTER_N>;
